...
function dispLog(log, n) {
	...
	for (var i = 0; i < log.length; i++) {
		...
		let temp = [log[i][0], log[i][1][0].innerText.split("-> ")[1].substr(0,30), log[i][1].attr("style")];
		console.log(temp);
		//console.log(log[i][1]);
		if (i >= 1) {break;}
	}
...
document.getElementById("draggableContainer").addEventListener("mouseleave", function() {
	...
	$("#draggableContainer")[0].innerHTML = stylesheet;
	//$("#draggableContainer").append(stylesheet);
